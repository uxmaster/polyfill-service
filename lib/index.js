...
function getPolyfillString(options) {
	...
	const lf = options.minify ? '' : os.EOL;
	...
	Promise.resolve(unsupportedUA ? {} : getPolyfills(options))
		...
		.then(targetedFeatures => {
			...
			output.queue('/* ' + explainerComment.join(`${os.EOL} * `) + ` */${os.EOL}${os.EOL}`);

			if (!sortedFeatures.length) {
				if (!options.minify) {
					output.queue(`/* No polyfills found for current settings */${os.EOL}`);
				}
				output.finalize();
				return;
			}
			...
			if ('all' in options.features) {
				output.queue(`${os.EOL}console.log(${allWarnText});${os.EOL}`);
			}
