...
/**
 * @returns {array} - Every polyfill that is available within the service.
 */
...
/**
 * @param {string} featureName - Name of a specific polyfill.
 * @returns {object|null} - Polyfill object or null if no polyfill found under the passed in featureName.
 */
...
/**
 * @param {object} opts - Custom options to override default options.
 * @returns {object} - Default options merged with passed in opts.
 */
 ...
 *
 * @param {object} options - Valid keys are uaString, minify, unknown, features, excludes and rum.
...
function getPolyfills(options) {
	...
	const resolveDependencies = createAliasResolver(
		function aliasDependencies(featureName) {
			const polyfill = sourceslib.getPolyfill(featureName);
	...
	const filterForUATargeting = function(features) {
		...
		const filteredList = featuresList.map(featureName => {
			const polyfill = sourceslib.getPolyfill(featureName);
	...
	return filterForExcludes(filterForUATargeting(resolveDependencies(filterForUATargeting(resolveAliases(options.features)))));
}


/**
 * @param {object} options - Valid keys are `uaString` {string}, `minify` {boolean}, `unknown` {string} (ignore|polyfill), `features` {object}, `excludes` {array} and `rum` {boolean}.
 * @returns {string} - The Javascript polyfill bundle
 */
function getPolyfillString(options) {
	...
	try {
		...
		const features = ((!ua.meetsBaseline() || ua.isUnknown()) && options.unknown !== 'polyfill') ? {} : getPolyfills(options);
